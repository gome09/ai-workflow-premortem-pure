...
from stages.prompts import INIT_SYSTEM, REVIEW_PROMPTS
...
def node_init(state: ProjectContext, user_input: str) -> ProjectContext:
    ...
    try:
        ...
        messages = [SystemMessage(content=INIT_SYSTEM)]
    ...
    if "✅ 信息收集完毕" in ai_text.replace("**", ""):
        ...
        target_match = re.search(r"\*{0,2}研究对象\*{0,2}[：:]\s*(.+)", ai_text)
        domain_match = re.search(r"\*{0,2}具体领域\*{0,2}[：:]\s*(.+)", ai_text)
        goal_match = re.search(r"\*{0,2}具体目标\*{0,2}[：:]\s*(.+)", ai_text)

        if target_match:
            state.research_target = target_match.group(1).strip()
        if domain_match:
