...
- Date: 2026-07-17
- By: claude-code (启动方式审计与偏差修复)
- Summary: 双子代理审计确认五种启动方式健全（Docker Lite/Full 双档），修复 6 处文档-机制偏差（startup/local_setup 过时测试数、README 中英文启动写法统一、Makefile 幽灵 dev target、demo-frontend env 刷新）。3 项工程决策项遗留（gen_secrets 孤儿脚本 / 生产栈 CI 覆盖 / prod-up fail-fast）。仓库仍处于"待维护者点公开按钮"状态。
