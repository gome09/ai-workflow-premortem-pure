...
2. 原 26 项验证支持 CRITICAL 风险项目（如用药管理）触发当时已有的高级门禁
3. 安全底线（缺失输出 / 解析错误 / 未决阻塞动作 / 拒绝动作 / 严重安全发现 / 依赖过期）在所有档位均强制阻断

## 后续能力说明

T3.3 的 `expert_review` 在上述原始风险自适应验证之后落地，不属于这份重建摘要的原始 26 项证据。CRITICAL 会话的专家复核阻断与放行行为由 `tests/test_expert_review_gate_v110.py` 单独验证。
