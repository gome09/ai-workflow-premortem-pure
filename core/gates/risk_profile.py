...
def classify_project_risk(ctx: ProjectContext) -> tuple[ProjectGateRiskTier, list[str]]:
    ...
    # 4. Low scope → lower one level, but NEVER below domain risk floor
    ...
    if low_hits:
        ...
        if tier == ProjectGateRiskTier.CRITICAL:
            ...
            reasons.append("低风险用途词不影响 CRITICAL 领域风险下限")
        elif tier == ProjectGateRiskTier.HIGH:
            # HIGH domain risk floor: low-scope keywords cannot downgrade
            reasons.append("低风险用途词不影响 HIGH 领域风险下限")
