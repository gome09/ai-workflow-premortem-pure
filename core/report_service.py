...
def build_report_dict(ctx: ProjectContext) -> dict:
    ...
    return {
        ...
        "deployment_decision": _dump(
            ctx.stage_4_output.deployment_decision if ctx.stage_4_output else None
        ),
        "demo_disclaimer": {
            "zh": "本报告为演示数据，未连接真实模型。压测证据多为演示证据（demo_evidence），不构成对真实模型的评估结论。",
            "en": "This report contains demo data not connected to real models. Stress test evidence is demo evidence and does not constitute a real model evaluation.",
            "note": "评估完成 ≠ 正式批准部署。",
        },
...
def build_markdown_report(ctx: ProjectContext) -> str:
    ...
    if ctx.stage_3_output and ctx.stage_3_output.test_results:
        for case in ctx.stage_3_output.test_results:
            ...
            ev_type = getattr(case, "evidence_type", "none")
            is_mock = getattr(case, "is_mock_evidence", False)
            if ev_type == "demo_evidence" or is_mock:
                lines.append(f"  - Evidence Type: {ev_type}（演示证据，未连接真实模型）")
            else:
                lines.append(f"  - Evidence Type: {ev_type}")
    ...
    lines.extend(["", "## 19. 部署门禁决策"])
    decision = ctx.stage_4_output.deployment_decision if ctx.stage_4_output else None
    if decision:
        lines.append(f"- Decision: {decision.decision}")
        lines.append(f"- Decision Scope: {decision.decision_scope}")
        if decision.decision_rationale:
            lines.append(f"- Decision Rationale: {decision.decision_rationale}")
        if decision.unresolved_risk_ids:
            lines.append(f"- Unresolved Risk IDs: {', '.join(decision.unresolved_risk_ids)}")
        if decision.required_conditions:
            lines.append("- Required Conditions:")
            for cond in decision.required_conditions:
                lines.append(f"  - {cond}")
        if decision.required_approvals:
            lines.append(f"- Required Approvals: {', '.join(decision.required_approvals)}")
        ...
        if decision.rollback_conditions:
            ...
            for cond in decision.rollback_conditions:
                lines.append(f"  - {cond}")
        if decision.prohibited_uses:
            lines.append("- Prohibited Uses:")
            for use in decision.prohibited_uses:
                lines.append(f"  - {use}")
        if decision.review_after:
            lines.append(f"- Review After: {decision.review_after}")
        if decision.human_accountable_role:
            lines.append(f"- Human Accountable Role: {decision.human_accountable_role}")
        lines.append(f"- Is Demo Recommendation: {decision.is_demo_recommendation}")
        if decision.is_demo_recommendation:
            lines.append("  - 注意：本决策为演示推荐，基于演示数据生成，未连接真实模型评测。")
    else:
        lines.append("No deployment decision recorded.")

    lines.extend(["", "## 20. Demo 声明"])
    lines.append("- 本报告为演示数据，未连接真实模型。")
    lines.append("- 压测证据多为演示证据（demo_evidence），不构成对真实模型的评估结论。")
    lines.append("- 评估完成 ≠ 正式批准部署。")
