...
def ensure_auth() -> None:
    """确保 session_state 里有可用的 access_token。

    优先尝试登录（/auth/login 限流 10/分钟，且 demo 账号在首次引导后就已存在，
    是绝大多数情况下的常见路径），登录失败（如首次运行、账号还不存在）时才
    回退到注册（/auth/register 限流 5/小时）。Streamlit 每次交互都会重跑整个
    脚本，若像早期版本那样优先调用注册接口，demo 账号已存在时每次重跑都会
    白白消耗一次宝贵的注册配额，很容易在短时间内触发 429，导致后续所有需要
    鉴权的接口都报 401。
    """
...
def handle_send(user_input: str, materials: list[str] | None = None) -> None:
    """
    统一处理发送消息的逻辑：
    追加用户消息（即时反馈）→ 调用 API → 从服务端权威历史整体重建消息列表 → 刷新状态

    阶段执行常在同一轮里产生多条 assistant 消息（结构化阶段输出 + 审核引导语），
    而 /chat 返回的 ai_reply 只带回其中时间戳最新的一条
    （见 core.session_service.SessionService._extract_latest_reply）。若只
    append 这一条 ai_reply，结构化阶段输出会静默丢失，不会显示在对话区——
    即使它已经正确落盘在服务端 conversation_history 里。因此这里改为整体
    重建，与切换会话时的 restore_messages_from_ctx 保持同一套「服务端历史
    是唯一真相源」的逻辑。
    """
