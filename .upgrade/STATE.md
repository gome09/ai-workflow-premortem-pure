...
- **缓存、临时产物与验收文档整理 (2026-08-15)** — 按用户确认删除 7 个未跟踪的一次性 demo 日志和历史 trace：`.upgrade/logs/demo-api.log`、`.upgrade/logs/demo-ui.log`、`.upgrade/traces/20260717-231500-mode3.trace`、`.upgrade/traces/20260717-234000-mode5.trace`、`.upgrade/traces/20260718-003000-mode2.trace`、`.upgrade/traces/20260720-094500-mode3.trace`、`.upgrade/traces/20260720-113000-mode3.trace`；保留仍被本状态记录引用的两份 trace。将 v1.2.1 验收快照与历史缺陷详录归档至 `docs/archive/verification-reports/acceptance-history-v1.2.1.md`，`docs/acceptance_report.md` 收缩为 v1.3.0 当前基线与回归摘要；README 启动说明压缩为入口与关键警告，并同步 `docs/README.md` 索引。既有 `.gitignore` / `.dockerignore` 缓存忽略规则改动一并保留。验证以文档检查、版本检查和 `git diff --check` 为准。
- **缓存与临时产物清理 (2026-08-15)** — 按用户确认仅清理项目级缓存和临时产物：删除 `.pytest_cache/`、`.mypy_cache/`、`.ruff_cache/`、项目内 `__pycache__/`、`.coverage` 与 `.upgrade/tmp/`；未改动 `artifacts/`、`data/`、`secrets/`、`nginx/certs/`、历史归档或升级日志/追踪目录。同步加强 `.gitignore` 与 `.dockerignore`，新增 `coverage/` 和 `pytestdebug.log` 忽略规则。当前运行缓存均可按命令重新生成。
...
- **Phase 3 Design Plan (2026-07-14)** — 详细设计方案 `docs/archive/plan/phase-3-design.md`
...
| 趋势报告 | `.upgrade/archive/reports/scorecard-trend-20260714.md` | ✅ |
...
- Summary: 文档整理与状态同步。Phase 0–4 全部历史计划文件归档至 `docs/archive/plan/`；.upgrade/ 已完成 Wave 计划与报告移至 `.upgrade/archive/`；对标调研数据快照删除；PIA 场景文档归档；STATE.md 更新为"仓库已公开"状态。
...
- Summary: 三路并行只读扫描（Mode 5 健康检查 / Mode 4 项目根扫描 / Mode 3 工作区审计）结论：根目录无杂散文件（两个 HTML 为已入库交付物，coverage/缓存/data 均被 .gitignore 正确覆盖，.env 未被 git 追踪）；工作区无硬错误。经用户确认执行两项整理：① 删除 `.upgrade/logs/` 全部 12 个一次性 CI 日志（~1.83MB，gitignored，分诊结论已固化于 `.upgrade/archive/reports/ci-run-20260718.md`）；② MANIFEST.md 修补规则悬空（FINAL_REPORT.md / reviews/ 标注"尚未产出"，补 research/ 非标准目录结构说明，登记 artifacts_live_e2e.log 已删）。plans/ 整批归档与 scorecard-trend 刷新继续 defer 至仓库公开 + Task 19 闭环后。

### 上一轮（2026-07-18）

- Date: 2026-07-18
- By: claude-code (本地 CI 复现 + 远端 GitHub CI 全绿验证)
- Summary: 本地按 ci.yml 复现 lint/testcov/lite/full 全部通过；远端首个 run 29621280076 失败后经 5b4003f 修复 doc-check 与 secrets 权限，最终 run（29647651072 / 收尾推送后 29647756391）三 job（lint-and-unit-tests / docker-lite-integration / docker-full-integration）全 success。完整报告 `.upgrade/archive/reports/ci-run-20260718.md`。仓库仍处于"待维护者点公开按钮"状态。
