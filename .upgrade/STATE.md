...
- Date: 2026-07-15
- By: claude-code (GitHub CI 离线全流程验证)
- Summary: 远端 `.github/workflows/ci.yml` 两个 job 实测全绿（lint-and-unit-tests + docker-lite-integration），全程离线 mock LLM + SQLite，无真实 LLM/外部 DB 依赖，CI run #13 conclusion=success。附带修复 owasp_agentic_2026 测试两处 docstring SyntaxWarning。本地验证：ruff clean、615 passed+8 skipped、version 1.2.1。CHANGELOG 追加维护记录条目。
