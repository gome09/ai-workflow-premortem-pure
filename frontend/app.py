...
def apply_stage_operation_result(result: dict | None) -> dict | None:
    ...
    Mutating v0.8 operations return domain payloads plus
...
def create_eval_experiment(
    ...
    result = api_post(
        f"/sessions/{session_id}/eval-experiments",
        {
            ...
            "name": name,
            "description": "Created from Streamlit v0.8-alpha.1 panel.",
            "run_mode": run_mode,
...
with st.sidebar:
        ...
        if st.button("➕ 新建会话", use_container_width=True):
            ...
            if sid:
                ...
                with st.spinner("加载引导语..."):
                    result = send_message(sid, "你好，我想开始一个新的项目分析。")
                if result:
                    st.session_state.messages.append(
                        {
                            "role": "assistant",
    ...
    if st.session_state.session_id:
        ...
        with st.expander("🧩 Execution / Interrupt Debug", expanded=False):
            ...
            st.caption(
                "v0.6.0-alpha.8：默认 single_step 保持稳定；"
        ...
        with st.expander("🧪 Eval Cases / Datasets / Experiments", expanded=False):
            ...
            st.caption(
                "v0.8-alpha.8: EvalDataset / EvalExperiment / Regression Gate / "
