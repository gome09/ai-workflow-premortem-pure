...
RULE_MANIFEST: dict[str, RuleMeta] = {
    ...
    "cross_stage_integrity": RuleMeta(
        rule_id="cross_stage_integrity",
        version="1.0.0",
        owner="project-owner",
        since_app_version="1.3.0",
        rationale="跨阶段 ID 引用必须有效，high/critical 风险必须有监督和测试覆盖，Stage 4 决策必须与门禁状态一致。",
        standard_refs=["INTERNAL:AI_GOV:CROSS_STAGE_INTEGRITY", "NIST_AI_RMF:MEASURE"],
        changelog=[("1.0.0", "2026-07-14", "初始版本")],
        safety_bottom_line=True,
    ),
}
