...
from core.deployment_decision_engine import apply_deployment_decision
    ...
    def parse_output(self, raw_text: str, ctx: ProjectContext) -> ProjectContext:
        if settings.stage_output_mode == "json_first":
            ...
            if result.parsed:
                ...
                apply_deployment_decision(ctx)  # 新增：生成部署决策
        ...
        # 生成确定性部署决策
        if ctx.stage_4_output:
            apply_deployment_decision(ctx)
