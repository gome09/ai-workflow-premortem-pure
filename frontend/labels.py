...
# ── Stage 4 部署门禁决策（DeploymentDecision.decision）──────────────────────
DECISION_ZH = {
    "go": "可部署",
    "conditional_go": "有条件部署",
    "no_go": "不可部署",
    "pilot_only": "仅试点",
    "conditional_deployment": "条件部署",
    "deployment_paused": "部署暂停",
}

# ── 部署决策范围（DeploymentDecision.decision_scope）────────────────────────
DECISION_SCOPE_ZH = {
    "limited_pilot": "有限试点",
    "full_deployment": "全面部署",
    "department_pilot": "部门试点",
}

# ── Stage 3 压测证据类型（StressTestResult.evidence_type）──────────────────
EVIDENCE_TYPE_ZH = {
    "demo_evidence": "演示证据（未连接真实模型）",
...
def decision_zh(value: object) -> str:
    return zh(DECISION_ZH, value)


def decision_scope_zh(value: object) -> str:
    return zh(DECISION_SCOPE_ZH, value)


def evidence_type_zh(value: object) -> str:
    return zh(EVIDENCE_TYPE_ZH, value)
