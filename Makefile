...
.PHONY: install clean dev-db dev-api dev-frontend dev docker-up docker-down lint test setup setup-win prod-up prod-down prod-logs demo-api demo-frontend demo-ui lite-up e2e-mock e2e-full-test e2e-live version-check doc-check audit security-check
...
# 实时四阶段 live E2E（需后端已启动，与 verify-demo.bat 等价，供 bash 用户使用）
e2e-live:
