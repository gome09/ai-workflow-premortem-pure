...
class TestSessionAPI:
    ...
    def test_create_session_with_scenario_id(self, client, mock_session_service, auth_headers):
        mock_svc = mock_session_service
        ctx = ProjectContext(selected_scenario_id="university_course_qa")
        mock_svc["create_session"].return_value = ctx

        response = client.post(
            "/sessions/",
            json={"scenario_id": "university_course_qa"},
            headers=auth_headers,
        )
        assert response.status_code == 200
        call_args = mock_svc["create_session"].call_args
        assert call_args.kwargs["scenario_id"] == "university_course_qa"
        assert response.json()["selected_scenario_id"] == "university_course_qa"
