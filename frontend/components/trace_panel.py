...
def render_trace_panel(traces: list[dict]) -> None:
    """Minimal trace explorer for v0.7 stage advancement finalization."""
    ...
    for trace in reversed(traces[-50:]):
        ...
        with st.expander(label, expanded=False):
            ...
            if metadata:
                st.json(metadata)
            st.caption(
                "v0.8-alpha.6: failed/parser/safety traces can be backfilled to EvalCase through the trace API."
            )
