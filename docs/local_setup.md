...
- `make setup` 随机生成 4 个密钥写入 `secrets/`：`jwt_secret` / `postgres_password` / `redis_password` / `grafana_password`。
...
- `DATA_ENCRYPTION_KEY` **不由 `make setup` 生成**。PostgreSQL 生产部署如需字段加密，须按 `.env.example` 注释自行生成 Fernet key 写入 `.env`，并在启动后确认 `/health` 的 `data_encryption` 为 `enabled`；为空时应用只告警并继续明文存储。
