# 历史归档：AI Workflow Premortem v1.2.1 验收与缺陷修复报告

> 本文件由原 `docs/acceptance_report.md` 中的 v1.2.1 全流程 E2E 快照与历史缺陷详录归档而来。数据按原始测试时间和版本理解，不代表当前实现基线；当前 v1.3.0 基线见 [`docs/acceptance_report.md`](../../acceptance_report.md)。
...
用户反馈“待处理人工动作已经全部完成，但依旧存在阻断”。前端显示“无待处理人工动作”，但阶段推进阻断器仍然存在。此问题涉及两个场景：
...
在 `resolve_action` 函数中，处理 `verify_evidence` 类型的人工动作时，只是将动作状态标记为 `RESOLVED`，但没有更新对应的 `evidence.verified` 字段。而证据门控（`stage1_evidence_gap.py`）检查的是 `evidence.verified`，不是动作是否已解决。

**问题2：红队测试覆盖门控**

`create_review_actions_for_stage` 函数中没有创建红队测试覆盖相关的人工动作。红队测试覆盖门控（`redteam_coverage.py`）会生成阻断器，但没有对应的人工动作让用户去处理，导致前端显示“无待处理人工动作”但阻断器仍然存在。

### 修复方案

**修复1：证据门控**

在 `core/oversight_service.py` 的 `resolve_action` 函数中添加了处理 `verify_evidence` 动作的逻辑：
...
1. 在 `core/oversight_service.py` 添加 `create_actions_from_redteam_gaps` 函数：
...
2. 在 `create_review_actions_for_stage` 中注册新函数
3. 在 `resolve_action` 中添加红队动作处理分支：
...
| 前端→后端 API 调用 | ✅ 16 个 panel 端点全部返回 200 |
...
1. **证据门控与人工动作联通**：处理 `verify_evidence` 类型的人工动作时，会自动更新对应证据的 `verified` 字段。
