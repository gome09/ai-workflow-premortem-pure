...
Windows 用户可直接使用仓库根目录的批处理脚本：

```
install-demo.bat   首次运行：检查 Python/uv、安装依赖、生成 .env
start-demo.bat     启动前后端（端口检查 + 后端健康轮询），就绪后自动打开浏览器
stop-demo.bat      按端口停止前后端服务（幂等）
verify-demo.bat    对已启动服务运行四阶段 live E2E 验证
```

流程：`install-demo.bat`（仅首次）→ `start-demo.bat` → 浏览器自动打开 http://localhost:8501。

- 前端界面：http://localhost:8501
- 后端 API：http://127.0.0.1:8000
- API 文档：http://127.0.0.1:8000/docs
- 登录：demo@example.com / demo-password-123

等价的 bash 命令：`make demo-api` / `make demo-ui`；live E2E 用 `make e2e-live`（或 `verify-demo.bat`）。
