...
- [ ] **Step 4: 用 Write 工具创建 `.upgrade/reports/pre-publication-checklist-20260717.md`（完整内容如下，三处 `<实测输出>` 用 Step 1–3 的实际输出回填）**
...
- [ ] **Step 1: pyproject.toml 加 pytest-cov 依赖**
...
- [ ] **Step 2: Makefile 加 test-cov target**
...
- [ ] **Step 4: ci.yml unit tests 步骤改为带覆盖率 + job summary**
...
- [ ] **Step 1: 用 Write 工具创建 `docs/plan/ecosystem-positioning.md`（完整内容如下，逐字）**
...
- [ ] **Step 2: `docs/README.md` plan 索引区追加一行**
...
- [ ] **Step 2: CHANGELOG.md 插入 v1.3.0 条目（追溯说明之后、`## 维护记录 (2026-07-16)` 之前）**
...
- [ ] **Step 3: 更新 `.upgrade/STATE.md`（六处 Edit，先 Read 全文）**
...
- [ ] **Step 4: 更新 `.upgrade/MANIFEST.md`（两处 Edit，先 Read）**
...
- [ ] **Step 5: 勾选父计划 Task 15–18 的 checkbox 并提交发布文件**
...
- [ ] **Step 6: 工作区收尾 commit + 打 tag（偏差 E-6：tag 打在收尾 commit）**
