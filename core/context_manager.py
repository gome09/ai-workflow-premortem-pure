...
def get_llm_for_stage(stage: int):
    """
    根据阶段返回对应模型。
    阶段一、三：DeepSeek V4 Pro + thinking，用于深度推理。
    阶段二、四：DeepSeek V4 Flash + non-thinking，用于结构化生成。

    When LLM_MODE=mock, returns MockLLMAdapter with no network calls.
    """
    if settings.llm_mode == "mock":
        from core.llm.provider import get_llm_client

        return get_llm_client(stage=stage, domain_profile=settings.domain_profile)
    return ChatOpenAI(**build_llm_kwargs_for_stage(stage))


def get_llm_for_stage_with_context(stage: int, ctx: ProjectContext):
    """Context-aware LLM selector.

    Scenario overrides remain session-scoped; default env-based behaviour is unchanged.
    """
    if settings.llm_mode == "mock":
        from core.llm.provider import get_llm_client

        return get_llm_client(stage=stage, domain_profile=current_domain_profile(ctx), ctx=ctx)
    return ChatOpenAI(**build_llm_kwargs_for_stage(stage))
