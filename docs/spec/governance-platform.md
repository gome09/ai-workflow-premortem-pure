...
> Status: Implemented（v1.2.0 落地 T3.1–T3.5、v1.2.0 落地 T3.7、v1.3.0 落地 T3.6（flag 默认关）。落地任务见 [../archive/plan/phase-3-governance-platform.md](../archive/plan/phase-3-governance-platform.md)）
...
- 数据口径：治理总览、门禁趋势、积压动作以及对应启动时业务 Gauge 只统计真实业务会话（`business_internal` / `sensitive_personal`）；`public_demo` 与带 `selected_scenario_id` 的四个内置场景会话防御性双重排除。总览响应的 `excluded_demo_sessions` 显示隔离数量，前端固定展示该口径；真实业务会话为零时展示空数据提示，接口异常则单独展示 HTTP/连接失败。
- 性能：租户内会话量级（内部工具，百级）直接实时聚合即可，不建物化视图。

### 4.2 前端治理页

Streamlit 新增"治理总览"页：指标卡片（项目数/积压动作/安全发现/报告）+ 状态与风险分布 + 通过率趋势折线 + 积压动作表格。页面顶部明确标注“仅真实业务数据、排除四个内置场景”，并显示本次隔离的演示会话数。

- 分布柱状图使用整数数量轴与灰色虚线横向参考网格，便于在低样本量时读取真实计数。
- 门禁趋势按周正序显示，纵轴固定为 0%–100%，不会因全部为 0% 而退化为零高度量程；数据点 tooltip 展示评估次数、通过次数与真实通过率。
- “8 周”表示查询窗口，不表示系统会补造没有评估记录的周。窗口内只有一个真实周桶时保留单点并明确提示“数据不足以形成趋势线”，不通过插值或零值填充制造趋势。
