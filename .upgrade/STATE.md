...
- Date: 2026-07-18
- By: claude-code (生产启动链路加固)
- Summary: 启动审计遗留项 7–9 全部落地：make setup 密钥自动随机化（gen_secrets.sh 接入 + .env 同步防遮蔽）、prod-up fail-fast 前置检查（prod-preflight）、生产栈 7 服务 CI 集成 job（non-blocking 观察期）。文档三处同步 + CHANGELOG 维护记录。docker-full-integration 需远端首轮验证后评估转 blocking。仓库仍处于"待维护者点公开按钮"状态。
