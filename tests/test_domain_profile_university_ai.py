...
class TestExampleFiles:
    def test_course_qa_example_exists(self):
        p = Path("examples/university_ai_course_qa_input.md")
        assert p.exists(), f"{p} not found"

    def test_course_qa_example_is_non_empty(self):
        p = Path("examples/university_ai_course_qa_input.md")
        assert p.stat().st_size > 100

    def test_course_qa_contains_all_required_fields(self):
        content = Path("examples/university_ai_course_qa_input.md").read_text(encoding="utf-8")
        for field in ("系统名称", "应用场景", "核心目标", "涉及数据"):
            assert field in content, f"course_qa missing field '{field}'"

    def test_course_qa_mentions_academic_integrity_risk(self):
        content = Path("examples/university_ai_course_qa_input.md").read_text(encoding="utf-8")
        assert "学术" in content or "诚信" in content or "作弊" in content
